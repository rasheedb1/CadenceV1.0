// Edge Function: Like a LinkedIn Post
// POST /functions/v1/linkedin-like-post
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Parse request body
    const body: LikePostRequest = await req.json()
    const { leadId, postId, postUrl, reactionType, cadenceId, cadenceStepId, scheduleId, instanceId, ownerId, orgId } = body
    ...
    // Like the post via Unipile
    const unipile = createUnipileClient()
    const result = await unipile.likePost({
    ...
    if (!result.success) {
      ...
      return errorResponse(result.error || 'Failed to like post')
    }
    ...
    await logActivity({
      ...
      action: 'linkedin_like',
      status: 'ok',
      details: { postId: targetPostId, reactionType: reactionType || 'LIKE' },
    })
