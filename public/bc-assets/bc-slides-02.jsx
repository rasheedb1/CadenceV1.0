...
function DownloadPdfButton({ clientName }) {
  ...
  const label = state === 'loading' ? 'generating PDF…' : state === 'error' ? 'try again' : 'download PDF';
...
function BCSlide16({ data }) {
  ...
  const breakdown = [
    { name: 'interchange optimization', bps: 21, pct: 55 },
    { name: 'multi-acquirer arbitrage', bps: 14, pct: 37 },
    { name: 'scheme fee optimization', bps: 3, pct: 8 },
  ];
  return (
    <div className="slide theme-ink" data-screen-label="16 Lever 2: MDR">
      ...
      <div style={{ position: 'absolute', top: 160, left: 80, right: 80 }}>
        ...
        <div className="t-subtitle-alt anim-in anim-in-1" style={{ color: 'rgba(255,255,255,0.5)', marginBottom: 40 }}>merchant discount rate · multi-acquirer routing + local processing + scheme optimization</div>
        <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 48 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
...
function BCSlide20({ data }) {
  setBCCurrency(data.currency);
  // Normalize legacy 'tiered' → 'tramos' so old presentations keep rendering identically.
  const model = data.pricingModel === 'flat'
    ? 'flat'
    : data.pricingModel === 'tiers' ? 'tiers' : 'tramos';
  const tierSubLabel = model === 'tramos'
    ? 'stacked · each bracket charges its own rate'
    : 'whole volume at the bracket rate';
  return (
      ...
      <div style={{ position: 'absolute', top: 160, left: 80, right: 80 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24, marginBottom: 32 }}>
          ...
          <div className="anim-in anim-in-1" style={{ padding: 32, borderRadius: 16, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            {model === 'flat' && (
              <React.Fragment>
                <div style={{ fontSize: 52, fontWeight: 300, color: '#fff', letterSpacing: '-0.02em', marginBottom: 8 }}>${fmtRate(data.ratePerTx)}<span style={{ fontSize: 16, color: 'rgba(255,255,255,0.4)' }}>/approved tx</span></div>
              ...
              <React.Fragment>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 4 }}>
                  {data.rateTiers.map((t, i) => {
                    // Brackets are stored annual (×12 from monthly user input). Display monthly to match how the user reasons about pricing.
                    ...
                    return (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)' }}>{capLabel} tx/mo</div>
          ...
          <div className="anim-in anim-in-2" style={{ padding: 32, borderRadius: 16, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
            {Number(data.reconciliationFee) > 0 ? (
              <React.Fragment>
                ...
                <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)' }}>flat monthly fee</div>
              </React.Fragment>
            ) : (
              <React.Fragment>
                <div className="t-label" style={{ color: '#8C99FF', marginBottom: 16 }}>minimum commitment</div>
                <div style={{ fontSize: 52, fontWeight: 300, color: '#fff', letterSpacing: '-0.02em', marginBottom: 8 }}>{fmtNum(Math.round(Number(data.minTxAnnual) / 12))}<span style={{ fontSize: 16, color: 'rgba(255,255,255,0.4)' }}> tx/mo</span></div>
                <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)' }}>{'≈'} {fmtMoney(Number(data.minCommitFee) / 12, { decimals: 1 })} /mo equivalent</div>
                <div style={{ fontSize: 12, color: '#8C99FF', marginTop: 14, paddingTop: 12, borderTop: '1px solid rgba(140,153,255,0.15)', fontWeight: 600, letterSpacing: '0.02em' }}>+ reconciliation: included in the pricing</div>
              </React.Fragment>
            )}
          </div>
          {/* Card 3: SaaS fee */}
          <div className="anim-in anim-in-3" style={{ padding: 32, borderRadius: 16, background: 'rgba(224,237,128,0.06)', border: '1.5px solid rgba(224,237,128,0.25)' }}>
            ...
            <div style={{ fontSize: 52, fontWeight: 300, color: '#E0ED80', letterSpacing: '-0.02em', marginBottom: 8 }}>{fmtMoney(data.monthlySaaS)}<span style={{ fontSize: 16, color: 'rgba(255,255,255,0.4)' }}>/mo</span></div>
            <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)' }}>platform + 24/7 support</div>
          </div>
        </div>
        {/* Your estimate */}
        <div className="anim-in anim-in-5" style={{ background: 'rgba(140,153,255,0.06)', border: '1px solid rgba(140,153,255,0.12)', borderRadius: 12, padding: '24px 32px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <div className="t-label" style={{ color: '#8C99FF' }}>your estimate {'·'} {fmtNum(Number(data.numActualTx) / 12)} transactions / mo</div>
            <div style={{ fontSize: 32, fontWeight: 300, color: '#fff' }}>{fmtMoney(Number(data.yunoAnnualFee) / 12, { decimals: 2 })}<span style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)' }}> /mo</span></div>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: 'rgba(255,255,255,0.55)', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 14 }}>
            <span>actual tx fee: {fmtMoney(Number(data.actualTxFee) / 12, { decimals: 2 })}</span>
            <span>min commit: {fmtMoney(Number(data.minCommitFee) / 12, { decimals: 2 })}</span>
            <span style={{ color: '#E0ED80' }}>tx (max): {fmtMoney(Number(data.txAnnualFee) / 12, { decimals: 2 })}</span>
            <span>+ saas: {fmtMoney(Number(data.saasAnnualFee) / 12, { decimals: 2 })}</span>
            {Number(data.reconciliationAnnualFee) > 0 && <span>+ reconciliation: {fmtMoney(Number(data.reconciliationAnnualFee) / 12, { decimals: 2 })}</span>}
          </div>
        </div>
      </div>
...
/* BCSlide20C — Additional services with prices.
   Renders the canonical ADDITIONAL_SERVICES list; each item reads
   data.additionalServices[id] = { enabled, price }. Disabled → "included in
   the pricing"; missing entry → enabled with default price. */
function BCSlide20C({ data }) {
  setBCCurrency(data.currency);
  const t = getServiceI18n(data.locale || 'en');
  const cur = data.currency || 'USD';
  const cfg = (data.additionalServices && typeof data.additionalServices === 'object') ? data.additionalServices : {};
  return (
    <div className="slide theme-ink-2" data-screen-label="20C Additional services">
      <div className="ink-grid" />
      <SectionLabel color="rgba(255,255,255,0.6)">04 / pricing · additional</SectionLabel>
    ...
    <div className="slide theme-ink-hero" data-screen-label="24 Close">
      ...
      <div style={{ position: 'absolute', top: '42%', left: 80, right: 80, transform: 'translateY(-50%)' }}>
        ...
        <div className="anim-in anim-in-4 no-print" style={{ display: 'flex', gap: 16, marginBottom: 56 }}>
          <a href={data.bookingUrl || `mailto:${data.salesEmail}?subject=${encodeURIComponent('Technical workshop · ' + (data.clientName || 'Yuno'))}`} target="_blank" rel="noopener noreferrer" style={{ background: '#E0ED80', color: '#0a0a1a', border: 'none', borderRadius: 10, padding: '16px 36px', fontSize: 16, fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.08em', textDecoration: 'none', display: 'inline-block' }}>schedule technical workshop</a>
          <DownloadPdfButton clientName={data.clientName} />
          {/* legacy plain link kept as fallback for users with JS disabled */}
          <noscript><a href={`https://bridge.yuno.tools/api/bc/${(window.BC_TRACKING && window.BC_TRACKING.slug) || ''}/pdf`} download>download PDF</a></noscript>
        </div>
