  ...
  const crearWorkflowAgente = tool(
    ...
    async ({ description: desc, name: wfName, activate }) => {
      try {
        ...
        const graphPrompt = `Generate a workflow graph JSON for this request:\n\n"${desc}"\n\nAVAILABLE AGENTS (use the exact id values for agentId):\n${agentContext}\n\nAVAILABLE NODE TYPES:\n- trigger_scheduled: {cron, timezone, label}\n- trigger_manual: {label}\n- action_agent_task: {agentId:"USE-UUID-FROM-LIST", agentName:"name", instruction:"DETAILED step-by-step instruction", maxBudgetUsd:2}\n- action_notify_human: {channel:"whatsapp", message:"...", label}\n\nARCHITECTURE RULES (CRITICAL):\n- PREFER A SINGLE action_agent_task WITH A DETAILED INSTRUCTION over multiple nodes. Agents are AI — they can follow multi-step instructions internally. Only split into multiple nodes when DIFFERENT agents handle different steps.\n- The instruction field should contain ALL the detailed steps the agent needs to follow, written as a numbered list.\n- A typical workflow is: trigger → one agent_task (with detailed multi-step instruction) → notify_human\n- Do NOT use condition_task_result — agents handle conditions internally.\n- agentId MUST be a UUID from the list above, NOT a name.\n- Every node MUST have a "label" in data.\n- Position nodes vertically (y increments of 150).\n\nReturn ONLY valid JSON: {"nodes":[...],"edges":[...]}`;
