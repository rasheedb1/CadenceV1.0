...
export default function SlideCaseLivelo({ pageNum, total, lang = 'es' }) {
  const t = (path) => tr(STRINGS, lang, path)
  const stats = STRINGS.caseLivelo.stats.map((s, i) => ({
    n: STAT_NUMBERS[i].n,
    suf: STAT_NUMBERS[i].suf,
    label: s.label[lang] || s.label.en,
  }))
  const actions = STRINGS.caseLivelo.actions.map((a) => a[lang] || a.en)

  return (
    <div className="slide theme-light">
      <SectionLabel>{t('caseLivelo.sectionLabel')}</SectionLabel>
      ...
      <h2 className="t-title t-title-m anim-in anim-in-1" style={{
        ...
        {t('caseLivelo.headlineLead')}
        <br/>
        <span style={{ color: 'var(--yuno-blue)' }}>{t('caseLivelo.headlineAccent')}</span>
      </h2>
      ...
      <SlideFooter section={t('caseLivelo.footerSection')} pageNum={pageNum} total={total} />
    </div>
  )
}
