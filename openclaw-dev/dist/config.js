...
function validateConfig() {
    const required = [
        ...
        ["TWILIO_WHATSAPP_NUMBER", exports.config.twilio.whatsappNumber],
    ];
    const missing = required.filter(([, v]) => !v).map(([k]) => k);
    if (missing.length > 0) {
        throw new Error(`Missing required env vars: ${missing.join(", ")}`);
    }
    if (!exports.config.oauth.refreshToken && !exports.config.anthropicApiKey) {
        throw new Error("Must set either OAUTH_REFRESH_TOKEN (Max plan) or ANTHROPIC_API_KEY");
    }
}
