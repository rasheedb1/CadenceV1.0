...
// --- OpenClaw Gateway integration via CLI ---
// Uses the OpenClaw CLI to send messages to the gateway.
// The CLI is already paired with the local gateway (onboard did this).
const fs = require("fs");
...
function sendToGateway(message, timeoutMs = 120000) {
  return new Promise((resolve, reject) => {
    ...
    execFile("node", [
      ...
      clearTimeout(timer);
      if (error) {
        console.error("[a2a] CLI error:", error.message, stderr?.substring(0, 200));
        reject(new Error(`CLI: ${error.message}`));
        return;
      }
      const output = (stdout || "").trim();
      console.log(`[a2a] CLI response: "${output.substring(0, 80)}"`);
