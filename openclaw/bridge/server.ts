...
const {
  ...
  BRIDGE_PORT = "3100",
  WEBHOOK_BASE_URL, // e.g. "https://your-app.railway.app" — needed for signature validation
} = process.env;
...
const connections = new Map<
  string,
  {
    ws: WebSocket;
    lastActivity: number;
    pendingReply: ((text: string) => void) | null;
  }
>();
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.on("message", (data: WebSocket.Data) => {
      try {
        ...
        // OpenClaw sends back assistant messages
        if (
          msg.type === "message" ||
          msg.type === "assistant" ||
          msg.type === "response"
...
/** GET — Twilio doesn't use GET verification, but return 200 for infra checks */
app.get("/api/whatsapp/incoming", (_req: Request, res: Response) => {
  res.status(200).json({
    status: "ok",
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!Body || !From) {
      // Acknowledge non-text messages (media, etc.) gracefully
      const twiml = new twilio.twiml.MessagingResponse();
      res.type("text/xml").send(twiml.toString());
      return;
    }

    // Immediately acknowledge receipt to Twilio (avoid timeout)
    // We'll send the AI response asynchronously via the REST API
    const twiml = new twilio.twiml.MessagingResponse();
    res.type("text/xml").send(twiml.toString());

    try {
      // Forward to OpenClaw and wait for reply
