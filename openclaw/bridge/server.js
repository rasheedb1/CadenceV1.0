...
1. FIRST decide: does the user want a ONE-TIME action or a RECURRING/AUTOMATED process?
   - ONE-TIME ("busca X", "genera Y", "dile a Z que haga W ahora") → resolver_skill + delegar_tarea
   - RECURRING ("todos los días", "cada semana", "diario", "automatiza", "rutina") → crear_workflow_agente
   - MULTI-STEP PROCESS ("primero haz X, luego Y, después Z") → crear_workflow_agente
...
- Multiple projects can run in parallel — select MINIMUM agents needed.`;
  ...
  const gwToolsLegacy = [
    ...
    { name: "delegar_tarea", description: "Delega una tarea ONE-TIME a un agente. Se ejecuta UNA sola vez, ahora. Usa para acciones inmediatas: 'busca X', 'genera Y', 'dile a Z que haga W'. NO uses para tareas recurrentes/diarias/semanales — esas van en crear_workflow_agente.", input_schema: { type: "object", properties: { org_id: { type: "string" }, agent_id: { type: "string", description: "ID del agente destino" }, agent_name: { type: "string", description: "Nombre del agente (alternativa a agent_id, búsqueda por nombre)" }, instruction: { type: "string", description: "La tarea en lenguaje natural" } }, required: ["org_id", "instruction"] } },
  ...
  const gwTools = [
    ...
    // --- Agent Workflows ---
    { name: "crear_workflow_agente", description: "Crea un workflow automatizado que se ejecuta de forma RECURRENTE o tiene MÚLTIPLES PASOS encadenados. Usa SIEMPRE que el usuario pida algo periódico ('todos los días', 'cada semana', 'diario', 'rutina', 'automatiza') o un proceso multi-paso ('primero X, luego Y, después Z'). El sistema genera el grafo visual automáticamente con los agentes, skills, schedule y condiciones necesarias. NUNCA delegues tareas recurrentes con delegar_tarea — siempre crear_workflow_agente.", input_schema: { type: "object", properties: { org_id: { type: "string" }, description: { type: "string", description: "Natural language description of the workflow. Be detailed about: which agents, what skills, what conditions, what happens on failure, schedule." }, name: { type: "string", description: "Short name for the workflow" }, activate: { type: "boolean", description: "Whether to activate immediately (default: false, saves as draft)" } }, required: ["org_id", "description", "name"] } },
